<form role="form" action="" method="post" name="dashboard_sales" id="dashboard_sales" enctype="multipart/form-data">
	...
	while ($row_sales=$sql1->fetch(PDO::FETCH_OBJ)) {
		...
		//get jumlah invoice per hari
		//$sqlstr = "select sum(a.total) total_value from sales_invoice a where a.date<='$date_sales' group by date_format(a.date,'%Y-%m')";

		$sqlstr2 = "select sum(a.total) total_value, month(a.date), a.date from sales_invoice a group by month(a.date), year(a.date) having year(a.date)='$year' and month(a.date)='$row_sales->kode'";
		$sql2=$dbpdo->prepare($sqlstr2);
		$sql2->execute();
		$row_total=$sql2->fetch(PDO::FETCH_OBJ);
		$total_value = numberreplace($row_total->total_value);

		if($total_value > 0) {
			if($string == '') {
				$string = '{
				    "hours": "' . $row_sales->name .'",
				    "value": "' . number_format($total_value,0,'','') .'",
				  }';
			} else {
				$string = $string .  ',{
				    "hours": "' . $row_sales->name .'",
	...
	<script type="text/javascript">
		...
		AmCharts.ready(function () {
			...
			categoryAxis.minPeriod = "YYYY"; //"YYYY"; // our data is yearly, so we set minPeriod to YYYY
			...
			graph.balloonText = "Omset:[[category]]<br><b><span style='font-size:14px;'>[[value]]</span></b>";
		...
		function zoomChart() {
			...
			chart.zoomToCategoryValues(1, 12);
		}
	</script>
