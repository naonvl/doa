...
class PDF extends FPDF
{
	...
	function PDF($orientation='P',$unit='mm', $format='qrcode') //tmsize //qrcode
	{
		...
		$this->FPDF2($orientation,$unit,$format,140,1660); //$size = tinggi //1660
		//Initialization
		$this->B=0;
		$this->I=0;
		$this->U=0;
	...
	function BasicTable($header,$data)
	{
		...
		for($t=0; $t<$count_print; $t++) {
			...
			if($t > 0) {
				$add = 0; //$add + 1;
				$qrcode_position = $qrcode_position + 39 + $add;

				$qrcode_position1 = $qrcode_position1 + 25 + $add;
			}
			//$size = $size + $sizeadd;		
		}
