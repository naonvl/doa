...
while($row_cash_invoice_detail=$sql2->fetch(PDO::FETCH_OBJ)) {
	...
	$item_name	=	substr($row_cash_invoice_detail->item_name,0,44);
...
class PDF extends FPDF
{
	...
	function Header()
	{
		...
		$this->SetFont('arial','',12);
		$this->Cell(36,5,'Nama',0,0,'L',true);
		$this->Cell(2,5,':',0,0,'L',false);
		$this->Cell(115,5,$client_name,0,0,'L',false);
		$this->Cell(20,5,'Tanggal : ' . $date,0,1,'L',false);
		//$this->Ln(2);
		
		$this->Cell(36,5,'Alamat Pengiriman',0,0,'L',true);
		$this->Cell(2,5,':',0,0,'L',false);
		$this->MultiCell(120,5,$ship_to,0,1,'L',true);
		//$this->Ln(2);
		
		$this->Cell(36,5,'Alamat Penagihan',0,0,'L',true);
		$this->Cell(2,5,':',0,0,'L',false);
		$this->MultiCell(120,5,$bill_to,0,1,'L',true);
		//$this->Ln(2);
		
		$this->Cell(36,5,'No. Telepon',0,0,'L',true);
		$this->Cell(2,5,':',0,0,'L',false);
	...
	function BasicTable($header,$data)
	{
		...
		foreach($header as $col) {
			...
			if ($i==2) { $this->Cell(92,6,$col,1,0,"C"); }
			if ($i==3) { $this->Cell(25,6,$col,1,0,"C"); }
			if ($i==4) { $this->Cell(25,6,$col,1,0,"C"); }
			if ($i==5) { $this->Cell(25,6,$col,1,0,"C"); }
			$i++;
		}
		$this->Ln();
		...
		{	
			...
			foreach($row as $col) {
				...
				if ($i==0) { $this->Cell(13,6,$col,1,0,"C"); }
				if ($i==1) { $this->Cell(14,6,$col,1,0,"C"); }
				if ($i==2) { $this->Cell(92,6,$col,1,0,"L"); }
				if ($i==3) { $this->Cell(25,6,$col,1,0,"R"); }
		...
		//$this->Cell(144,5,'',0,0,'L',true);	
		$this->Cell(169,5,'Sisa',0,0,'R',false);
		...
		$this->Cell(49,5,'Petugas',0,1,'C',true);	
		$this->Ln(10);
		...
		$this->Cell(20,5,'BANK',0,0,'L',true);
		$this->Cell(2,5,':',0,0,'L',false);
		$this->Cell(50,5,$bank_name,0,1,'L',true);
		
		$this->Cell(20,5,'NO. REK',0,0,'L',true);
		$this->Cell(2,5,':',0,0,'L',false);
		$this->Cell(50,5,'',0,1,'L',true);
		
		$this->Cell(20,5,'A/N',0,0,'L',true);
		$this->Cell(2,5,':',0,0,'L',false);
		$this->Cell(50,5,'',0,1,'L',true);
...
$header=array('Qty','Satuan','Nama Barang','Nilai','Discount','Jumlah');
