        ...
        <div class="col-xs-12">
            ...
            <?php
                if($_POST["submit"] == 'Upload') {
                    ...
                    $uid    =   $_SESSION['loginname']; //'import_'.
                    ...
                    {    
                        ...
                        //if($x >= 0) {
                            ...
                            if($ref2 != "" && $client_name != "") {
                                ...
                                $date           =   $data->val($i, 2); //date('Y-m-d', strtotime($data->val($i, 2)));
                                ...
                                $qty            =   numberreplace(trim($data->val($i, 10)));
                                $unit_price     =   numberreplace(trim($data->val($i, 11)));
                                $discount       =   numberreplace(trim($data->val($i, 12)));
                                ...
                                if($rowssales == 0) {
                                    ...
                                    $status = "Paid";
                                    $paid = 1;
                                    $sqlstr = "insert into sales_invoice (ref, ref2, date, status, client_code, ship_to, receipt_type, bank_account, paid, uid, dlu) values ('$ref', '$ref2', '$date', '$status', '$client_code', '$address', '$receipt_type', '$bank_account', '$paid', '$uid', '$dlu')";
                                    $sql=$dbpdo->prepare($sqlstr);
                                    $sql->execute();
