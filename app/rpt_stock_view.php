...
<table width="100%" border="0" cellpadding="0" cellspacing="0" style="font-family: Arial; font-size: 12px">
	<tr>
		<td align="left"><b>LAPORAN STOK BARANG</b></td>
	</tr>
...
<table width="100%" border="1" cellpadding="0" cellspacing="0" style="border:#ccc; font-family: Arial; font-size: 12px">	
	...
	<?php
		...
		while($row_bincard=$sql->fetch(PDO::FETCH_OBJ)) {
			...
			if($invoice_type == "stockopname") {
				...
				//non SO
				/*$sql1=$selectview->rpt_stock_stock_opname_limit1($row_bincard->item_code, $row_bincard->location_code, $row_bincard->uom_code, $all, $row_bincard->item_group_id, $row_bincard->item_subgroup_id, $date_from, $date_to, "");
				$data1=$sql1->fetch(PDO::FETCH_OBJ);
				$invoice_type=$data1->invoice_type;
				
				if($invoice_type != "stockopname") {
					$sql2=$selectview->rpt_non_stock_opname($row_bincard->item_code, $row_bincard->location_code, $row_bincard->uom_code, $all, $row_bincard->item_group_id, $row_bincard->item_subgroup_id, $date_stok_opname, $date_to);
					$data2=$sql2->fetch(PDO::FETCH_OBJ);
					$debit_qty = $data2->debit_qty;
					$credit_qty = 0;
	...
	?>
			...
			<tr <?php echo $bgcolor; ?> >
				<td align="left" bgcolor="">&nbsp;<?php echo $row_bincard->location_name  ?></td>
				<td align="left" bgcolor="">&nbsp;<?php echo $row_bincard->code  ?></td>
