        ...
        <div class="col-xs-12">
            ...
            <?php
                if($_POST["submit"]) {
                    ...
                    for ($i=0; $i<=$baris; $i++)
                    {    
                        echo $i.'>>: '.$data->val($i, $x) .'<br>';

                        //if($i == 0) {

                            //echo str_replace("/","-",$data->val($i, 2)).'>>';
                            /*$date           =   str_replace("/","-",$data->val($i, 2));
                            $date           =   date('Y-m-d', strtotime($date));
                            $location_id    =   numberreplace($data->val($i, 4));

                            echo $date.'>>'.$item_code.'>>'.$uom_code.'>>'.$qty.'<br>';*/
                            //echo $location_id.'<br>';
                        //} 
                        /*else {
                            $item_code      =   petikreplace($data->val($i, 2));
                            $uom_code       =   petikreplace($data->val($i, 4));
                            $qty            =   numberreplace($data->val($i, 5));
                            $unit_cost      =   numberreplace($data->val($i, 7));
                        }*/
